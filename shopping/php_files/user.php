<?php
	...
	if(isset($_POST['create'])){
		...
		}else if(!isset($_POST['rpassword']) || empty($_POST['rpassword'])){
			echo json_encode(array('error'=>'Confirm Password Field Empty.')); exit;
		}else if(!isset($_POST['mobile']) || empty($_POST['mobile'])){
			echo json_encode(array('error'=>'Mobile Number Field Empty.')); exit;
		}else if(($_POST['password'] != $_POST['rpassword']) || empty($_POST['password'])){
			echo json_encode(array('error'=>'Passwords not Match.')); exit;
	...
	if(isset($_POST['login'])){
		if(!isset($_POST['username']) || empty($_POST['username'])){
			...
			$db->select('user','user_id,username,user_role,f_name,l_name',null,"username = '{$username}' AND password = '{$password}'",null,null);
			$response = $db->getResult();
			if(!empty($response)){
	...
	if(isset($_POST['update'])){
		if(!isset($_POST['f_name']) || empty($_POST['f_name'])){
			...
			$db->update('user',$paramsl,"user_id = '{$user_id}'");
			$response = $db->getResult();
			if(!empty($response)){
	...
	if(isset($_POST['modifyPass'])){
		...
		}elseif(!isset($_POST['rnew_pass']) || empty($_POST['rnew_pass'])){
			echo json_encode(array('error'=>'Confirm New Passowrd field Empty')); exit;
		}elseif(($_POST['new_pass'] != $_POST['rnew_pass']) || empty($_POST['new_pass'])){
			echo json_encode(array('error'=>'Passwords not Match.')); exit;
		}else{
			$db = new Database();
